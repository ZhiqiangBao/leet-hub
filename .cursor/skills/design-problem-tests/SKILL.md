...
7. **语言中立**。测例是 JSON，对 Python / C / C++ 同一份。避免依赖「整数溢出当 unsigned」或 Python 无限整数才对的数据；值域跟题面走。默认 C 的 `int`（32 位有符号）；仅签名为 `long` 的字段才用 64 位。
8. **C 可表示**。`List[int]` 不要用超过 32 位有符号的整数；`List[long]` / 返回 `long` 可到 int64。字符串不要嵌 `\0`；返回数组题的 `expected` 长度要与题意一致。
