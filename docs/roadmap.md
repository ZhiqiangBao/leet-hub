...
当前已有：登录、题库（搜索 / 难度 / 知识点筛选）、做题页、Python / C / C++20 评测、测试（只跑公开示例）与提交分开、草稿、个人提交记录与成绩排行、管理员统计与全站提交、管理员网页增题、题面 `$…$` KaTeX、整站浅色 / 深色背景切换。签名默认 `int`（int32），少数字段用 `long`。题库真源仍是 GitHub 的 `problems/`。出题走 Qwen 流水线（`QWEN.md`），不要复制力扣原题。
...
| 语言 | 工具链 | 安装体积（约） | 说明 |
| --- | --- | --- | --- |
| JavaScript（`javascript`） | Node.js | linux-x64 `.tar.xz` **34 MB** | 已有桩。`node --check` / `node solution.js`，标准库 `JSON`，禁止 npm 包。 |
| TypeScript（`typescript`） | 同上 Node + `tsc` | `typescript` npm 包几十 MB | **未做。** 官方 `tsc` 跑在 Node 上，不另装一套编译器。`tsc` 编成 JS 再 `node` 跑。与 JS 共用 Node。 |
