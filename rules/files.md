...
工具会拆掉示例三行外的代码围栏、统一全角冒号、值同行，把 `nums = [1,3,2,4], hi = 5` 改成位置参数 `[1, 3, 2, 4], 5`，并生成 starter、补上缺的 bounds 字段。返回 JSON 含 `examples.edits`（第几条、input/layout、是否 kv_to_positional）与 `bounds_notes`。
