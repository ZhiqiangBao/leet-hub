...
def check_statement(root: Path, slug: str) -> dict:
    ...
    ref_path = root / ".qwen" / "tmp" / f"{slug}_ref.py"
    starters = {
    ...
    if solve is None:
        out["ok"] = False
        out["issues"].append("[答案] missing ref.py solve()")
    saw32 = saw64 = False
    for args, expected in parsed:
        ...
        if solve is not None and (st != "ok" or not values_equal(got, expected, compare)):
...
def main() -> int:
    ...
    out = check_statement(Path(args.root), args.slug)
